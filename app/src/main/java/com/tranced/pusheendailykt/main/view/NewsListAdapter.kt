...
class NewsListAdapter(
    ...
    override fun getItemViewType(position: Int): Int {
        when (position) {
            Constants.FIRST_ITEM_POSITION -> return Constants.BANNER_TYPE
            newsItems.size + 1 -> return Constants.LOADING_TYPE
            else -> {
                when (newsItems[position - 1].type) {
                    Constants.DATELINE_SYMBOL -> return Constants.DATELINE_TYPE
                    else -> return Constants.NEWS_ITEM_TYPE
                }
            }
        }
    }
    ...
    private fun setBannerListeners(viewHolder: BannerViewHolder) {
        viewHolder.banner.setOnTouchListener { v, event ->
            when (event.action) {
    ...
    class MyHandler(viewHolder: BannerViewHolder) : Handler() {
