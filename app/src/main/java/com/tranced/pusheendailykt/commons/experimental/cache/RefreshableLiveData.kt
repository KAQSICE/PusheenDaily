package com.tranced.pusheendailykt.commons.experimental.cache

import android.content.Context
import androidx.lifecycle.LiveData
import com.tranced.pusheendailykt.commons.experimental.extensions.QuietCoroutineExceptionHandler
...
//    val stateLiveData: LiveData<ConsumableMessage<RefreshState<I>>> = mutableState
    ...
    abstract fun refresh(
        vararg indicators: I,
        callback: suspend (RefreshState<CacheIndicator>) -> Unit = {}
    )
...
fun HttpException.handleError(block: HttpExceptionHandler) {
    try {
        ...
        val err_code = jsonObject.getInt("error_code")
...
fun Context.simpleCallback(
    success: String? = "加载成功",
    error: String? = "发生错误",
    refreshing: String? = "加载中"
): suspend (RefreshState<*>) -> Unit =
    with(this.asReference()) {
        {
            when (it) {
                is RefreshState.Success -> if (success != null) Toasty.success(this(), success)
                    .show()
                is RefreshState.Failure -> if (error != null) Toasty.error(
                    this(),
                    "$error ${it.throwable.message}！${it.javaClass.name}"
                ).show()
                is RefreshState.Refreshing -> if (refreshing != null) Toasty.normal(
                    this(),
                    "$refreshing..."
                ).show()
            }
...
fun <V : Any> RefreshableLiveData.Companion.use(local: Cache<V>, remote: Cache<V>) =
    object : RefreshableLiveData<V, CacheIndicator>() {
        ...
        override fun refresh(
            vararg indicators: CacheIndicator,
            callback: suspend (RefreshState<CacheIndicator>) -> Unit
        ) {
...
fun <V : Any> RefreshableLiveData.Companion.use(
    local: Cache<V>,
    remote: Cache<V>,
    callbackOnEach: suspend (V) -> Unit
) =
    object : RefreshableLiveData<V, CacheIndicator>() {
...
fun <V : Any> RefreshableLiveData.Companion.retrofit(remote: Cache<V>) =
    object : RefreshableLiveData<V, CacheIndicator>() {
        private var running: Job? = null
