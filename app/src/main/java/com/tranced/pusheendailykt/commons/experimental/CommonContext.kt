package com.tranced.pusheendailykt.commons.experimental
        ...
        activityClasses[name]?.let {
            context.startActivity(
                Intent(
                    context,
                    it
                ).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK).apply(block)
            )
        }
...
fun Context.startActivity(name: String, block: Intent.() -> Unit = {}) =
    CommonContext.startActivity(this, name, block)